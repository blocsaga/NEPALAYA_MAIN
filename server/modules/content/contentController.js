...
contentController.GetContentForUsers = async (req, res, next) => {
  try {
    let { page, size, populate, selectQuery, searchQuery, sortQuery } = otherHelper.parseFilters(req, 10, false);
...
contentController.GetContentForCounter = async (req, res, next) => {
  try {
    var doc = 0;
    contentSch.findByIdAndUpdate({ _id: 'visitor_counter' }, { $inc: { seq: 1 } }, { new: true, upsert: true }, function (error, contentSch) {
      if (error) return next(error);
      doc = (contentSch && contentSch.seq) || 1;
...
// contentController.GetContentByUser = async (req, res, next) => {
//   const _id = req.user;
//   console.log(_id);
//   console.log('_id');
//   try {
...
contentController.selectMultipleData = async (req, res, next) => {
  ...
  if (type == 'is_active') {
    const Data = await contentSch.updateMany({ _id: { $in: content_id } }, [
      {
        $set: {
